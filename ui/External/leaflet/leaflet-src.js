...
L.Util.extend(L.DomUtil, {
	TRANSITION: L.DomUtil.testProp(['webkitTransition', 'transition', 'OTransition', 'MozTransition', 'msTransition']),
...
L.DomEvent = {
    ...
    _filterClick: function(e, handler) {
        // are they closer together than 500ms?
        // Android typically triggers them ~300ms apart
        var diff = e.timeStamp - L.DomEvent._lastClick;
        if (diff > 0 && diff < 500) {
            L.DomEvent.stop(e);
            return;
        }
...
L.Transition = L.Transition.extend({
	...
	_onBackupFireEnd: function () {
		...
		event.initEvent(L.DomUtil.TRANSITION_END, true, false);
...
if (L.DomUtil.TRANSITION) {
	L.Map.addInitHook(function () {
		L.DomEvent.on(this._mapPane, L.DomUtil.TRANSITION_END, this.catchTransitionEnd, this);
	});
}
